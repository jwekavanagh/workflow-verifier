...
**Plan transition (git + plan markdown):** `node dist/cli.js plan-transition --repo <dir> --before <ref> --after <ref> --plan <path>` — validates `git diff -z --name-status` against rules from either YAML front matter key **`planValidation`** or a single **`## Repository transition validation`** body section whose first fence is **`yaml`**/**`yml`**, same **`schemaVersion`/`rules`** shape ([`plan-validation-core.schema.json`](schemas/plan-validation-core.schema.json)). Cursor-style front matter without rules must include one of those sources. Emits **`WorkflowResult`** (default **`workflowId` `wf_plan_transition`**). Requires **Git ≥ 2.30.0**. Spec: [Plan transition validation (normative)](docs/execution-truth-layer.md#plan-transition-validation-normative).
